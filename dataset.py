...
def split_data(data_path):
    datas=open(data_path,encoding="utf-8").readlines()
...
class EnglishChineseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        src=self.tokenizer(src,padding="max_length",max_length=self.max_sql_len,truncation=True,return_tensors="pt")["input_ids"]
        dst_in="<s>"+dst
        dst_in=self.tokenizer(dst_in,padding="max_length",max_length=self.max_sql_len,truncation=True,return_tensors="pt")["input_ids"] 
        dst_label=self.tokenizer(dst,padding="max_length",max_length=self.max_sql_len,truncation=True,return_tensors="pt")["input_ids"]
